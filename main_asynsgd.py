...
def main():
    ...
    model = [ models.__dict__[args.model] for i in range(node_num + 1)]
    ...
    # add N optimizer for workers and 1 for master
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_result = train(train_loader, model, criterion, epoch, optimizer, U, V, timestamp_Mtx)

        train_loss, train_prec1, train_prec5, U, V, timestamp_Mtx = [
            train_result[r] for r in ['loss', 'prec1', 'prec5', 'U', 'V', 'timestamp_Mtx']]
...
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None, U=None, V=None, timestamp_Mtx=None):
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        if not training:
            ...
            if(0 == i):
                ...
                print('number of models is ', len(model))

            # fjr

            Num_works = len(mini_inputs)

            # fjr simulate distributed senario, acc_grad should be a N*N matrix,
            # each elem is parameters of the model

            # TODO initialize for master
            mini_target_var = mini_targets[0]
            output = model[Num_works](mini_inputs[0])
            ...
            for k, mini_input_var in enumerate(mini_inputs):
                ...
                if args.use_delayed_sgd:
                    # send model[k] grad to master
                    for pmaster, pwk in zip(model[Num_works].parameters(), model[k].parameters()):
                        pmaster.grad.data.zero_()
                        # TODO check whether to divide N
                        pmaster.grad.data = pwk.grad.data.clone() / Num_works
                    # clip_grad_norm(model[Num_works].parameters(), 5.)

                    # master update  model
                    optimizer[Num_works].step()

                    # copy model from master
                    for pmaster, pwk in zip((model[Num_works].parameters()), model[k].parameters()):
                        pwk.data = pmaster.data.clone()

            if args.use_delayed_sgd == False:
...
def train(data_loader, model, criterion, epoch, optimizer, U, V, timestamp_Mtx):
...
def validate(data_loader, model, criterion, epoch):
    ...
    for i in range(len(model)):
        model[i].eval()
    return forward(data_loader, model, criterion, epoch,
                   training=False, optimizer=None, U=None, V=None, timestamp_Mtx=None)
